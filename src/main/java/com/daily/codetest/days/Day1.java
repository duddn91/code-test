...
public class Day1 {
	...
	public int main(String str1, String str2) {
		...
		for (String word : strSet) {
			...
			IntStream.range(0, min).mapToObj(i -> word).forEach(intersection::add);
			IntStream.range(0, max).mapToObj(i -> word).forEachOrdered(union::add);
		}

		return jaccardSimilarity(intersection.size(), union.size()).intValue();
	}
	...
	private Double jaccardSimilarity(int x, int y) {
		double defaultValue = 65536.0;
		if (x == 0 && y == 0) {
			return defaultValue;
		}

		return Math.floor(((double) x / y) * defaultValue);
	}
}
